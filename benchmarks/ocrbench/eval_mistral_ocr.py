...
# ///
"""
OCRBench evaluation script for Mistral OCR API
Usage: uv run eval_mistral_ocr.py --api-key YOUR_KEY
"""
...
def run_inference(client: Mistral, model: str, image_path: str, question: str) -> str:
    """Run inference using Mistral OCR API."""
    ...
    if result.pages:
        text = result.pages[0].markdown if result.pages[0].markdown else ""
        return text.strip()
    return ""
...
def main():
    parser = argparse.ArgumentParser(description="OCRBench evaluation for Mistral OCR API")
    parser.add_argument("--api-key", type=str, required=True, help="Mistral API key")
    parser.add_argument("--model", type=str, default="mistral-ocr-latest", help="Model name")
    ...
    parser.add_argument("--output", type=str, default="./results/mistral_ocr.json", help="Output file")
