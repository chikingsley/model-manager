...
# ///
"""
OCRBench evaluation for Mistral Vision models (VQA tasks)
Uses chat API with vision for document Q&A.

Usage:
    uv run eval_mistral_vqa.py
    uv run eval_mistral_vqa.py --model mistral-large-latest
"""
...
def run_inference(client: Mistral, model: str, image_path: str, question: str) -> str:
    """Run VQA inference using Mistral vision API."""
...
def main():
    parser = argparse.ArgumentParser(description="OCRBench evaluation for Mistral Vision")
    ...
    if not args.api_key:
        raise ValueError("API key required. Set MISTRAL_API_KEY in .env or use --api-key")
